...
import (
	...
	routingpg "llm-gateway/backend/services/routing-service/internal/infra/postgres"
	routinghttp "llm-gateway/backend/services/routing-service/internal/interfaces/http"
)
...
func main() {
	...
	repo := routingpg.NewRepository(pool)
	if err := repo.EnsureSchema(ctx); err != nil {
		logger.Error("failed to ensure routing schema", "error", err)
		os.Exit(1)
	}

	svc := routingapp.NewServiceWithRepository(repo)
	...
	routinghttp.NewHandler(svc).RegisterRoutes(engine)
