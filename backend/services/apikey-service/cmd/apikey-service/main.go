...
import (
	...
	apikeypg "llm-gateway/backend/services/apikey-service/internal/infra/postgres"
	apikeyhttp "llm-gateway/backend/services/apikey-service/internal/interfaces/http"
)
...
func main() {
	...
	logger := logx.New("apikey-service", cfg.LogLevel)
	...
	repo := apikeypg.NewKeyRepository(pool)
	if err := repo.EnsureSchema(ctx); err != nil {
		logger.Error("failed to ensure apikey schema", "error", err)
		os.Exit(1)
	}

	svc := apikeyapp.NewServiceWithRepository(repo)

	engine := ginx.NewEngine(logger, cfg.Environment)
	engine.GET("/healthz", ginx.HealthHandler("apikey-service"))
	...
	apikeyhttp.NewHandler(svc).RegisterRoutes(engine)
